...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	public partial class GLTFSceneImporter : IDisposable
	{
		...
		private IEnumerator CustomWaitUntil(Func<bool> predicate)
		{
			while (!predicate())
			{
				yield return null;
		...
		private async Task LoadJson(string jsonFilePath)
		{
			...
			if (IsMultithreaded && dataLoader2 != null)
			{
				...
				loadThread.Start();
				RunCoroutineSync(CustomWaitUntil(() => !loadThread.IsAlive));
			}
			else
			{
			...
			if (IsMultithreaded)
			{
				...
				RunCoroutineSync(CustomWaitUntil(() => !parseJsonThread.IsAlive));
				if (_gltfRoot == null)
				{
		...
		protected async Task _LoadScene(int sceneIndex = -1, bool showSceneObj = true, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			if (IsMultithreaded)
				await Task.Run((Action)PrepareUnityMeshData, cancellationToken);
			else
			...
			{
				Debug.LogError(String.Empty, e.ToString());
			}
		}
		...
		private async Task<GameObject> GetNode(int nodeId, CancellationToken cancellationToken)
		{
			try
			{
				if (_assetCache.NodeCache[nodeId] == null)
				{
					...
					{
						Debug.LogError(String.Empty, ex.ToString());
					}
		...
		private async Task<(Vector3, Quaternion, Vector3)[]> GetInstancesTRS(Node node)
		{
			    ...
			    node.Extensions.TryGetValue(EXT_mesh_gpu_instancing_Factory.EXTENSION_NAME, out var ext))
			{
				...
				async Task<AttributeAccessor> GetAttrAccessorAndAccessorContent(AccessorId accessorId, bool isPosition = false)
				{
					...
					unsafe {
						var bufferPointer = bufferViewCache.GetUnsafePtrAlt();
						switch (accessor.Type)
						{
							case GLTFAccessorAttributeType.VEC3:
								if (isPosition)
									attrAccessor.AccessorId.Value.AsVertexArray(ref resultArray, bufferPointer);
								else
									attrAccessor.AccessorId.Value.AsFloat3Array(ref resultArray, bufferPointer);
								break;
							case GLTFAccessorAttributeType.VEC4:
								attrAccessor.AccessorId.Value.AsFloat4Array(ref resultArray, bufferPointer);
								break;
						}
					}
				...
				if (extMeshGPUInstancing.attributes.TryGetValue(EXT_mesh_gpu_instancing.ATTRIBUTE_ROTATION, out var rotationAccessorId))
				{
					...
					{
						Debug.LogError(String.Empty, "Rotation attribute count does not match position attribute count for instances!");
						return null;
					}
					else
				...
				if (extMeshGPUInstancing.attributes.TryGetValue(EXT_mesh_gpu_instancing.ATTRIBUTE_SCALE, out var scaleAccessorId))
				{
					...
					{
						Debug.LogError(String.Empty, "Scale attribute count does not match position attribute count for instances!");
						return null;
					}
					else
