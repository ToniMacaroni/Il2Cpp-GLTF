	...
	public partial class GLTFSceneImporter
	{
		...
		async Task<Texture2D> CheckMimeTypeAndLoadImage(GLTFImage image, Texture2D texture, NativeArray<byte> data, bool markGpuOnly, bool isLinear)
		{
			switch (image.MimeType)
			{
				...
				case "image/jpeg":
					...
					texture.LoadImage(data.ToArray() as Il2CppStructArray<byte>, markGpuOnly);
					break;
				case "image/exr":
					GLTFSceneImporter.Debug.Log(LogType.Warning, $"EXR images are not supported. The texture {texture.name} won't be imported. File: {_gltfFileName}");
					break;
				case "image/ktx2":
...
#endif
					{
						GLTFSceneImporter.Debug.Log(LogType.Warning, $"Can't import texture \"{image.Name}\" from \"{_gltfFileName}\" because it is a KTX2 file using the KHR_texture_basisu extension. Add the package \"com.unity.cloud.ktx\" version v1.3+ to your project to import KTX2 textures.");
		...
		protected virtual async Task ConstructUnityTexture(Stream stream, bool markGpuOnly, bool isLinear, bool isNormal, GLTFImage image, int imageCacheIndex)
		{
			...
			if (stream is FileLoader.InvalidStream invalidStream)
			{
				...
				texture = null;
				GLTFSceneImporter.Debug.Log(LogType.Error, "Buffer file " + invalidStream.RelativeFilePath + " not found in path: " + invalidStream.AbsoluteFilePath+ $" (File: {_gltfFileName})");
			}
			else
			...
			if (_assetCache.ImageCache[imageCacheIndex] != null) GLTFSceneImporter.Debug.Log(LogType.Assert, "ImageCache should not be loaded multiple times");
			if (texture)
			{
				progressStatus.TextureLoaded++;
				progress?.Report(progressStatus);
		...
		protected virtual async Task ConstructTexture(GLTFTexture texture, int textureIndex, bool markGpuOnly, bool isLinear, bool isNormal)
		{
			if (_assetCache.TextureCache[textureIndex].Texture == null)
			{
				...
				if (texture.Sampler != null)
				{
					...
					switch (sampler.MinFilter)
					{
						...
						case MinFilterMode.LinearMipmapLinear:
							...
							break;
						default:
							GLTFSceneImporter.Debug.Log(LogType.Warning, "Unsupported Sampler.MinFilter: " + sampler.MinFilter+ $" (File: {_gltfFileName})");
							desiredFilterMode = FilterMode.Bilinear;
							break;
					}

					TextureWrapMode UnityWrapMode(WrapMode gltfWrapMode)
					{
						switch (gltfWrapMode)
						{
							...
							default:
								GLTFSceneImporter.Debug.Log(LogType.Warning, "Unsupported Sampler.Wrap: " + gltfWrapMode+ $" (File: {_gltfFileName})");
								return TextureWrapMode.Repeat;
						}
					}
				...
				if (matchSamplerState || markGpuOnly)
				{
					...
					{
						GLTFSceneImporter.Debug.Log(LogType.Warning, $"Ignoring sampler; filter mode: source {source.filterMode}, desired {desiredFilterMode}; wrap mode: source {source.wrapModeU}x{source.wrapModeV}, desired {desiredWrapModeS}x{desiredWrapModeT}"+ $" (File: {_gltfFileName})");
					}
				}
				else
#if UNITY_EDITOR
...
#endif
				{
					...
					if (!source.isReadable)
					{
						...
						unityTexture = Object.Instantiate<Texture2D>(source);
