...
#region Profiler Markers
		...
		private NodeId ExportNode(Transform nodeTransform)
		{
			...
			{
				var uniquePrimitives = GLTFSceneExporter.GetUniquePrimitivesFromGameObjects(primitives);
							...
							if (blendShapeWeights != null)
							{
								if (data.weights != null)
								{
									...
									for (int i = 0; i < blendShapeWeights.Count; i++)
										isSame &= System.Math.Abs((double)(blendShapeWeights[i] - data.weights[i])) < double.Epsilon;
									if (!isSame)
...
#region Public API
		...
		{
			var materialKey = 0;
			if (materialObj == GLTFSceneExporter.DefaultMaterial)
				materialKey = 0;
			else if (materialObj)
		...
		{
			if (_textureSettingsToSamplerIndices.TryGetValue(new GLTFSceneExporter.SamplerRelevantTextureData(textureObj), out var samplerId))
			{
